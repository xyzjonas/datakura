...
1. warehouse item evidence while receiving (inbound warehouse order) - we can select either package, serialized item or fungible (no tracking). In addition a batch identifier can be attached to that item. That is possible even for a fungible item - making it partially trackable. In the UI form however, this option is bugged out as if the select's value is 'fungible' it automatically skips the server call and makes the button only close the dialog without making any changes. Correct behavior is that if 'batch' is applied to a fungible item, proper server calls have to be made rendering a preview and POSTing the evidence change to the backend when hitting the 'evidovat' button.
